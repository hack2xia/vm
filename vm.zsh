...
vm_scan() {
  ...
  #     清单里存的大小写未必和磁盘一致（本机 WinSer2019 就是），以磁盘为准才不会
  #     把同一台机器认成两台。
  ...
  # 3b. 再用 Fusion 清单补全：路径大小写权威，且能覆盖默认目录之外的 VM
  ...
  case "$cmd" in
    ...
    status)
      ...
      if [[ -n "$name" ]]; then
        ...
        for name in ${(ok)VM_VMX}; do
          if (( ${running[(Ie)${VM_VMX[$name]}]} )); then
            mark="%F{green}●%f"; state="%F{green}运行中%f"
          else
            mark="%F{white}○%f"; state="未运行"
          fi
    ...
    vms)
      ...
      for n in ${(ok)VM_VMX}; do
        p="${VM_VMX[$n]}"
        d="${${VM_DISPLAY[$n]}//\%/%%}"
        print -rP "  %F{cyan}${(r:14:)${n//\%/%%}}%f${(r:14:)d} ${p//\%/%%}"
      done
      ;;
    scan)
      vm_scan
